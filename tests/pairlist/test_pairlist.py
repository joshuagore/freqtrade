...
@pytest.mark.parametrize("pairlists,base_currency,whitelist_result", [
    ...
    ([{"method": "VolumePairList", "number_assets": 20, "sort_key": "quoteVolume"},
      {"method": "PriceFilter", "low_price_ratio": 0.02}],
        "USDT", ['ETH/USDT', 'NANO/USDT']),
    ([{"method": "StaticPairList"},
      {"method": "RateOfChangeFilter", "lookback_days": 10,
...
def test_rateofchangefilter_checks(mocker, default_conf, markets, tickers):
    default_conf['pairlists'] = [{'method': 'VolumePairList', 'number_assets': 10},
                                 {'method': 'RateOfChangeFilter', 'lookback_days': 99999}]
    ...
    with pytest.raises(OperationalException,
                       match=r'RateOfChangeFilter requires lookback_days to not exceed '
                             r'exchange max request size \([0-9]+\)'):
        get_patched_freqtradebot(mocker, default_conf)

    default_conf['pairlists'] = [{'method': 'VolumePairList', 'number_assets': 10},
                                 {'method': 'RateOfChangeFilter', 'lookback_days': 0}]

    with pytest.raises(OperationalException,
                       match='RateOfChangeFilter requires lookback_days to be >= 1'):
        get_patched_freqtradebot(mocker, default_conf)
...
def test_rateofchangefilter_caching(mocker, markets, default_conf, tickers, ohlcv_history_list,
                                    min_rate_of_change, expected_length):
    default_conf['pairlists'] = [{'method': 'VolumePairList', 'number_assets': 10},
                                 {'method': 'RateOfChangeFilter', 'lookback_days': 2,
...
@pytest.mark.parametrize("pairlistconfig,desc_expected,exception_expected", [
    ...
    ({"method": "RateOfChangeFilter", "lookback_days": 10, "min_rate_of_change": 0.01},
     "[{'RateOfChangeFilter': 'RateOfChangeFilter - Filtering pairs with rate of change below 0.01 "
     "over the last days.'}]",
        None
     ),
])
